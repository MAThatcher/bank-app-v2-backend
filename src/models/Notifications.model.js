...
const Preferences = require('../services/PreferencesService');
...
function list(userId, options = {}, extra = {}) {
    return prisma.notifications.findMany({
        where: { user_id: Number(userId), in_app: true, ...extra,
...
module.exports = {
    ...
    getNotificationById: async (userId, notificationId) => wrapRows(await prisma.notifications.findFirst({
        where: { user_id: Number(userId), id: Number(notificationId), in_app: true },
        select: publicFields,
    })),
    ...
    createNotification: async (message, userId, tx = prisma, type = 'general') => {
        const channels = await Preferences.channels(userId, type, tx);
        return wrapRows(await tx.notifications.create({
            data: { message, user_id: Number(userId), type, dismissed: false, in_app: channels.inApp,
                email_status: channels.email ? 'pending' : 'disabled' }, select: publicFields,
        }));
    },
    dismissAllNotifications: async userId => wrapRows(await prisma.notifications.updateMany({
        where: { user_id: Number(userId), in_app: true, ...unread }, data: { dismissed: true, update_date: new Date() },
    })),
    getUnreadNotifications: (userId, options) => list(userId, options, unread),
    getUnreadCount: userId => prisma.notifications.count({ where: { user_id: Number(userId), in_app: true, ...unread } }),
