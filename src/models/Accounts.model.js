...
module.exports = {
    ...
    getAccountUsersByAccountId: async (accountId) => {
        const rows = await prisma.account_users.findMany({ where: { account_id: Number(accountId), archived: Boolean(false) },});
        return wrapRows(rows);
    },
    ...
    insertAccount: async (accountName, ownerId, tx = prisma) => {
        const created = await tx.accounts.create({ data: { name: accountName, owner: Number(ownerId) }, select: { id: true }, });
        return { rows: [{ id: created.id }] };
    },

    insertAccountUser: async (accountId, userId, tx = prisma) => {
        await tx.account_users.create({ data: { account_id: Number(accountId), user_id: Number(userId) }, });
        return { rows: [] };
    },
    ...
    archiveAccountUsers: async (accountId, tx = prisma) => {
        await tx.account_users.updateMany({ where: { account_id: Number(accountId) }, data: { archived: true, update_date: new Date() }, });
        return { rows: [] };
    },

    archiveAccount: async (accountId, tx = prisma) => {
        await tx.accounts.update({ where: { id: Number(accountId) }, data: { archived: true, update_date: new Date() }, });
        return { rows: [] };
    },

    archiveTransactionsByAccount: async (accountId, tx = prisma) => {
        await tx.transactions.updateMany({ where: { account_id: Number(accountId) }, data: { archived: true, update_date: new Date() }, });
        return { rows: [] };
    },

    getAccountByOwnerAndId: async (userId, accountId) => {
        const rows = await prisma.accounts.findMany({ where: { owner: Number(userId), archived: false, id: Number(accountId) }, });
        return wrapRows(rows);
    },
    ...
    findAccountUserIdByEmail: async (accountId, email) => {
        const user = await prisma.users.findFirst({ where: { email, archived: false, account_users: { some: { account_id: Number(accountId) } } }, select: { id: true }, });
        if (!user) return { rows: [] };
